...
public:
    ...
    statusPub(ArClientBase *client, ros::NodeHandle *nh,
              std::string name = "Pose", std::string topic = "/pose");
    // The laser callback is called when data is sent from the robot and
    // publishes to ROS
    ...
    void battery_voltage_cb(ArNetPacket *packet);
    void current_draw_cb(ArNetPacket *packet);
    ...
    void LocaliseCallback(
        const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &msg);
    void moveExecteCB(const move_base_msgs::MoveBaseGoalConstPtr &goal);

    bool requestDock(ros_omron_agv::DockRequest::Request &req,
                     ros_omron_agv::DockRequest::Response &res)
    {
        res.result = true;
...
protected:
    ...
    ArFunctor1C<statusPub, ArNetPacket *> currentDrawCB;
    ArFunctor1C<statusPub, ArNetPacket *> batteryVoltageCB;
    ...
    actionlib::SimpleActionServer<move_base_msgs::MoveBaseAction>
        as_; // Make a simple move to pose action server
...
statusPub::statusPub(ArClientBase *client, ros::NodeHandle *nh,
                     std::string name, std::string topic)
    : myClient(client),
      _nh(nh),
      ...
      currentDrawCB(this, &statusPub::current_draw_cb),
      batteryVoltageCB(this, &statusPub::battery_voltage_cb),
      dockedStatusCB(this, &statusPub::dock_stats_cb),
      myStatusCB(this, &statusPub::status_cb),
      as_(*_nh, "move_base", boost::bind(&statusPub::moveExecteCB, this, _1),
          false)
{
    ...
    sub_goal = _nh->subscribe("/move_base_simple/goal", 10,
                              &statusPub::simplePoseCallback, this);
    sub_initpose =
        _nh->subscribe("/initialpose", 10, &statusPub::LocaliseCallback, this);
    ros::spinOnce;

    myClient->addHandler("updateNumbers", &myPoseCB);
    myClient->request("updateNumbers",
                      50); // Seems if we request rate of 50 we get 10hz max
    ...
    // Battery Stuff
    myClient->addHandler("DataStore_Battery1CurrentDraw", &currentDrawCB);
    myClient->request("DataStore_Battery1CurrentDraw", period);

    myClient->addHandler("DataStore_Battery1Voltage", &batteryVoltageCB);
    myClient->request("DataStore_Battery1Voltage", period);

    ROS_INFO("Setup Callback for %s publishing on %s", name.c_str(),
             topic.c_str());
    ...
    // Cmd vel
    cmdVelSub = _nh->subscribe("cmd_vel", 10, &statusPub::cmdVelCB,
...
void statusPub::current_draw_cb(ArNetPacket *packet)
{
    char out[20];
    packet->bufToStr(out, 20);
    currentDraw = std::atof(&out[1]);
}

void statusPub::battery_voltage_cb(ArNetPacket *packet)
{
    char out[20];
    packet->bufToStr(out, 20);
    batteryVoltage = std::atof(&out[1]);
}
...
void statusPub::pos_x_cb(ArNetPacket *packet)
{
    ...
    q.setRPY(0, 0,
             angles::from_degrees(
                 posTheta /
                 1.0)); // TODO this is bit shit there is better numbers around
...
{
    double chargePerc, x, y, theta, x_vel, y_vel, theta_vel, temp;

    // packet->printHex();
    chargePerc = ((double)packet->bufToByte2()) / 10.0;
    ...
    data.batteryPercentage = chargePerc;
    data.batteryVoltage = batteryVoltage;
    data.currentDraw = currentDraw;
    ...
    return;
    // std::cout << chargePerc << " " << x << " " << y << " " << theta <<
    // std::endl; static tf::TransformBroadcaster br; tf::Transform transform;
    // transform.setOrigin( tf::Vector3(x/1000.0, y/1000.0, 0.0) );
    // tf::Quaternion q;
    // q.setRPY(0, 0, angles::from_degrees(theta/1.0)); //TODO this is bit shit
    // there is better numbers around transform.setRotation(q);
...
void statusPub::status_cb(ArNetPacket *packet)
{
    ...
    pch = strstr(
        myStatus,
        "Parking"); // When it is in an idle state the robot retruns parking
    if (pch != NULL)
    {
    ...
    pch = strstr(
        myStatus,
        "Failed: Failed going to goal"); // Couldn't make it e.g. path blocked
    if (pch != NULL)
    {
        robotStatus = actionlib_msgs::GoalStatus::ABORTED;
    }
    pch = strstr(myStatus,
                 "Failed: Cannot find path"); // Couldnt find valid plan
    if (pch != NULL)
    {
...
void statusPub::dock_stats_cb(ArNetPacket *packet)
{
    ...
    if (secondsToShutdown == 0)
        ROS_INFO("State: %s Forced: %s Shutdown: never", stateStr.c_str(),
                 forcedStr.c_str());
    else
        ROS_INFO("State: %s Forced: %s Shutdown: %d", stateStr.c_str(),
                 forcedStr.c_str(), secondsToShutdown);
}

void statusPub::simplePoseCallback(
    const geometry_msgs::PoseStamped::ConstPtr &msg)
{
    //<td>gotoPose</td> <td> X (4-byte integer), Y (4-byte int), Theta (optional
    // 2-byte int)</td>
...
void statusPub::LocaliseCallback(
    const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &msg)
{
    //<td>gotoPose</td> <td> X (4-byte integer), Y (4-byte int), Theta (4-byte
    // int)</td>
...
void statusPub::cmdVelCB(const geometry_msgs::Twist::ConstPtr &msg)
{
    ...
    if (vx > MAX_FORWARD_SPEED || vx < -MAX_REVERSE_SPEED ||
        fabs(omega) > MAX_ROT_SPEED)
    {
        vel_valid = false;
        ROS_INFO(
            "Commanded velocity over greater than max, velocity is in m/s! Are "
            "you sure??");
        ROS_INFO("Max Forward: %f, Max Reverse: %f, Commanded: %f",
                 MAX_FORWARD_SPEED, MAX_REVERSE_SPEED, vx);
    ...
    if (fabs(vx) > 2 || fabs(omega) > 2) // These are in % here
    {
        ...
        packet.doubleToBuf(
            100); // this is an additional amount (percentage) that is applied
                  // to each of the trans,rot,lat velocities.
...
void statusPub::cmdVelWD(const ros::TimerEvent &)
{
    ...
    else if (vel_valid == true)
    {
        vel_valid = false;
        myClient->requestOnce("stop");
        ROS_WARN("Timeout on cmd_vel. velCount: %d, prevVelCount: %d", velCount,
                 prevVelCount);
    }
}
